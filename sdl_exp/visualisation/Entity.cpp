...
void Entity::importModel(const char *path)
{
	...
	{
		fprintf(stderr, "File %s uses floats of %ull bytes, this architecture has floats of %zu bytes. Aborting import\n", importPath.c_str(), mask.SIZE_OF_FLOAT, sizeof(float));
		fclose(file);
		return;
	}
	if (sizeof(unsigned int) != mask.SIZE_OF_UINT)
	{
		fprintf(stderr, "File %s uses uints of %ull bytes, this architecture has floats of %zu bytes. Aborting import\n", importPath.c_str(), mask.SIZE_OF_UINT, sizeof(unsigned));
		fclose(file);
		return;
	}
