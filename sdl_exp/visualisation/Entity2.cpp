    ...
    , materialBuffer(std::make_shared<UniformBuffer>(sizeof(MaterialProperties)))
{
    ...
    {
        materials[0].setDiffuse(glm::vec3(1));
    }
    //bind attribs
    if (shaders)
    {
        ...
        shaders->setMaterialBuffer(materialBuffer);
        shaders->setFaceVBO(this->faces.vbo);
    }
    for (auto &&m : materials)
    {
        ...
        if (this->vertices.data)
        {
            ...
            it->setPositionsAttributeDetail(this->vertices);
            it->setNormalsAttributeDetail(this->normals);
            it->setColorsAttributeDetail(this->colors);
            it->setTexCoordsAttributeDetail(this->texcoords);
            it->setMaterialBuffer(materialBuffer);
            it->setFaceVBO(this->faces.vbo);
        }
    }
}
Entity2::~Entity2()
...
void Entity2::loadModel()
{
 ...
 //       }
    ...
    if (shaders)
    {
        ...
        shaders->setModelMatPtr(this->getModelMatPtr());
    }
    //printf("Model : %s was loaded.\n%dv, %dvn, %dvc, %dvt, %df, %d/%dmat, %llurg\n", modelPath.c_str(), vertices.count, normals.count, colors.count, texcoords.count, faces.count / faces.components, materials.size() - missingMats, materials.size(), renderGroup.size());
    printf("Model : %s was loaded.\n%dv, %dvn, %dvc, %dvt, %df, %llurg\n", modelPath.c_str(), vertices.count, normals.count, colors.count, texcoords.count, faces.count / faces.components, renderGroup.size());
}
void Entity2::reload()
{
