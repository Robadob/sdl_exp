...
{    
    ...
    //Setup companion
    resizeBackBuffer(this->companionWindowDims);
    companionLeft = std::make_shared<Sprite2D>(std::dynamic_pointer_cast<Texture2D>(vr_leftResolveFB->getColorTexture()), std::make_shared<Shaders>(Stock::Shaders::SPRITE2D_NOALPHA), glm::uvec2(companionWindowDims.x / 2, companionWindowDims.y));
    companionRight = std::make_shared<Sprite2D>(std::dynamic_pointer_cast<Texture2D>(vr_rightResolveFB->getColorTexture()), std::make_shared<Shaders>(Stock::Shaders::SPRITE2D_NOALPHA), glm::uvec2(companionWindowDims.x / 2, companionWindowDims.y));
    hud->add(companionLeft, HUD::AnchorV::South, HUD::AnchorH::West, glm::ivec2(0), -1024);
    hud->add(companionRight, HUD::AnchorV::South, HUD::AnchorH::East, glm::ivec2(0), -1024);
    //Load render models
...
void VisualisationVR::resizeWindow()
{
    ...
    if (companionLeft)
        companionLeft->setDimensions(glm::uvec2(this->companionWindowDims.x / 2, this->companionWindowDims.y));
    if (companionRight)
        companionRight->setDimensions(glm::uvec2(this->companionWindowDims.x / 2, this->companionWindowDims.y));
}
...
const glm::mat4 *VisualisationVR::getProjectionMatPtr() const{
	if (this->vr_renderModels)
        return this->vr_renderModels->getCamera()->getProjectionMatPtr();
	return nullptr;
}
glm::mat4 VisualisationVR::getProjectionMat() const{
	if (this->vr_renderModels)
        return this->vr_renderModels->getCamera()->getProjectionMat();
	return glm::mat4();
}
